...
{                               /* --- find frequent item sets */
  ...
  &&  (fim->supps[0] >= fim->smin)) {
    ...
    if (r > 0) {                /* if item 0 needs processing */
      isr_report(fim->report);  /* report the current item set */
      isr_remove(fim->report,1);/* and remove the item 0 again */
    ...
    if (r > 0) {                /* if item 1 needs processing, */
      ...
      &&  (fim->wgts[3] >= fim->smin)) {
        if (fim->wgts[3] >= fim->supps[1])
          ...
          if (r < 0) return -1; /* add item 0 to the reporter */
          if (r > 0) {          /* if item 0 needs processing */
            isr_report(fim->report);
            isr_remove(fim->report, 1);
          }                     /* report the current item set */
        }                       /* and remove the item 0 again */
      }                         /* from the item set reporter */
      isr_report(fim->report);  /* report the current item set */
      isr_remove(fim->report,1);/* remove item 1 again from */
  ...
  &&  (fim->supps[2] >= fim->smin)) {
    ...
    if (r > 0) {                /* if  item 2 needs processing, */
      ...
      isr_report(fim->report);  /* report the current item set */
      isr_remove(fim->report,1);/* remove item 2 again from */
    }                           /* the item set reporter */
  }
  ...
  &&  (fim->supps[3] >= fim->smin)) {
    ...
    if (r > 0) {                /* if  item 3 needs processing, */
      ...
      if (r < 0) return -1;     /* find freq. item sets recursively */
      isr_report(fim->report);  /* report the current item set */
      isr_remove(fim->report,1);/* remove item 3 again from */
    }                           /* the item set reporter */
  }
  ...
  for (i = 4; i < n; i++) {     /* traverse the (remaining) items */
    ...
    isr_report(fim->report);    /* report the current item set */
    isr_remove(fim->report, 1); /* remove item i again from */
  }                             /* the item set reporter */
  return 0;                     /* return 'ok' */
}  /* rec_pos() */
...
{                               /* --- find frequent item sets */
  ...
  &&  (fim->supps[1] >= fim->smin)) {
    ...
    if (r > 0) {                /* if item 1 needs processing, */
      ...
      isr_report(fim->report);  /* report the current item set */
      isr_remove(fim->report,1);/* and remove the item 1 again */
    }                           /* from the item set reporter */
  }
  if ((mask & 1)                /* if item 0 is frequent */
  &&  (fim->supps[0] >= fim->smin)) {
...
{                               /* --- main function */
  ...
  #endif  /* #ifndef QUIET */
  /* free option characters: acijlopy [A-Z]\[CT] */
  ...
  isr_report(report);           /* report the empty set (if needed) */
