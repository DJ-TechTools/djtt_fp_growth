/*----------------------------------------------------------------------
  ...
  History : 2004.11.21 file created from eclat.c
            ...
            2013.05.16 added ruby API (drummondj)
------------------------------------------------------------------------
...
static VALUE run_fp_growth(VALUE self, VALUE options) {
  const int argc = RARRAY_LEN(options);
  const char* argv[argc];
 
  int i;
  for(i=0; i<argc; i++) {
    VALUE option = rb_ary_entry(options, i);
    argv[i] = StringValueCStr(option);
    printf("%s\n", argv[i]);
  }
  
  return INT2NUM(main(argc,argv));
}

void Init_djtt_fp_growth(void) {
  VALUE klass = rb_define_class("DjttFpGrowth", rb_cObject);
  rb_define_singleton_method(klass, "run_fp_growth", RUBY_METHOD_FUNC(run_fp_growth), 1);
}