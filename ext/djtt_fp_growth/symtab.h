...
#define int         1           /* to check definition of INDEX */
...
#endif
#ifndef IDENT_FMT
#define IDENT_FMT   "ld"        /* printf format code for int */
#endif
...
#  else
#  define IDENT_FMT "zd"        /* printf format code for ptrdiff_t */
...
extern void        my_st_delete  (SYMTAB *tab);
extern void*       my_st_insert  (SYMTAB *tab, const void *key, int type,
                              size_t keysize, size_t datasize);
extern int         st_remove  (SYMTAB *tab, const void *key, int type);
extern void*       my_st_lookup  (SYMTAB *tab, const void *key, int type);
...
/*--------------------------------------------------------------------*/
#ifdef IDMAPFN
#define idm_delete(m)     my_st_delete(m)
#define idm_add(m,n,k,s)  my_st_insert(m,n,0,k,s)
#define idm_byname(m,n)   my_st_lookup(m,n,0)
#define idm_bykey(m,k)    my_st_lookup(m,k,0)
